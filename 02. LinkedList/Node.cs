...
namespace _02._LinkedList
{
    internal class Node
    {
		...
		// < 노드 구현 >
		// 노드에 데이터와 다른 노드들을 참조하는 값을 구성하여 구현

		// 연결된 노드를 가지는 일정갯수의 변수 또는 배열을 통해 고정적인 연결구조를 가짐
        // 연결구조가 일정한 연결리스트 / 트리 등에 사용

		public class LinkedListNode<T>  // 노드의 구성
		{
			public T value;  // 데이터 값
			public LinkedListNode<T> Prev; // 이전 데이터의 위치
			public LinkedListNode<T> next; // 다음 데이터의 위치
		}
        ...
        public class OctTreeNode<T>     // 옥타트리
        {
            public T value;
